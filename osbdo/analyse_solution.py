...
def plot_finite_memory(*, memory_bounds, memory, h_true, iters=None, \
    ...
    num_cl = len(memory)+1
    color = sns.color_palette("hls", num_cl)
    ...
    for i in range(len(memory)):
        ...
        start_point = 0
        for j in range(min(iters, Uk.size)):
...
def plot_uk_rel_gap_uk(*, uk, h_true, title='', eps={'eps_rel': 10**(-2), \
                        'eps_abs': 10**(-3)}, y_min=None, y_max=None, fontsize=10,  file_name=None):
    Uk = np.array(uk)
    start_point = 0
    for j in range(len(Uk)):
    ...
    for i in range(len(Uk)):
        if not flag and (Uk[i]-h_true < eps['eps_abs'] \
                            or (Uk[i]-h_true < eps['eps_rel']*min(np.abs(Uk[i]),np.abs(h_true)) and Uk[i]*h_true>0)):
            stop = i
            print(f"{i=}, omega_true={gap1[i]}")
            break

    ax1.axvline(stop, color='orange', linestyle=':')
    ax1.plot(gap1,linewidth=lwd,label=r'$\omega_{true}^{k}$',color='b')
    ax1.set_xlabel(r'$k$',fontsize=fontsize)
    ax1.set_yscale('log')
    if y_min is not None and y_max is not None:
